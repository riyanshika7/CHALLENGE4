...
def dijkstra_path(start: str, end: str, accessibility: dict, db_locations: list) -> dict:
    ...
    if rerouted_stairs and (is_wheelchair or is_stroller):
        explain_parts.append(
            "Step-free routing enforced: completely bypassed stairs-only areas (e.g., Gate B/Section 102) to prioritize elevators/ramps.")
    if rerouted_crowd:
        explain_parts.append(
            "High capacity bypass active: rerouted around gates and concourses reporting >80% crowd bottlenecks to minimize delay.")
...
def navigate_simulator(start: str, end: str, crowd_level: str, accessibility: dict) -> dict:
    ...
    if is_visual:
        ...
        route_steps.append(
            "Follow the tactile paving strips on the floor. Braille maps are located at the West Info desk.")
...
def handle_navigation(start: str, end: str, crowd_level: str, accessibility: dict, db_locations: list = None) -> dict:
    ...
    if is_testing:
        ...
        try:
            ...
            for loc in db_locations:
                loc_strings.append(
                    f"- Name: {loc.name}, Type: {loc.type}, Accessibility Features: {loc.accessibility_features}, Crowd Level: {loc.crowd_level}, Description: {loc.description}")
            context = "\n".join(loc_strings)
