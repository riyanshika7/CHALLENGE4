...
def generate_insights_simulator(locations_data: list, incidents_data: list, weather_warnings: list) -> dict:
    ...
    if high_density_zones:
        problems.append(
            f"Imminent gridlock warning at {', '.join(high_density_zones)}. Inflow rate exceeds exit corridor capacity.")
        actions.append(
            f"Deploy volunteer teams immediately to pre-routing checkpoints 50m upstream of {', '.join(high_density_zones)} to divert fans.")
    ...
    active_meds = sorted(
        list(set(inc["location"] for inc in incidents_data if inc["category"] == "medical" and inc["status"] == "open")))
    if active_meds:
        problems.append(
            f"Elevated stress and crowd slowdowns near Section {', '.join(active_meds)} due to active medical triaging.")
        actions.append(
            f"Direct standby volunteers to form a safety perimeter around the medical zone in Section {', '.join(active_meds)} to keep corridors clear.")
        workflows.append(
            f"Automated System Action: Dispatched emergency response vehicle to VIP access gate near Section {', '.join(active_meds)}.")
    ...
    if weather_warnings:
        ...
        workflows.append(
            "Automated System Action: Requested facilities cleanup crew to deploy high-traction mats on all external exit ramps.")
...
def handle_ambient_insights(db: Session) -> dict:
    ...
    loc_list = [{"name": loc.name, "crowd_level": loc.crowd_level, "crowd_factor": loc.crowd_factor}
                for loc in locations]

    # 2. Fetch active incidents
    incidents = db.query(Incident).filter(Incident.status == "open").all()
    inc_list = [{"category": inc.category, "location": inc.location,
                 "urgency": inc.urgency, "status": inc.status} for inc in incidents]
